...
@RestController
@RequestMapping("/api/{customerId}/budgets/{budgetId}/transactions")
public class TransactionController {
    ...
    @GetMapping
    public ResponseEntity<List<Transaction>> getAllTransactions(@PathVariable long customerId, @PathVariable long budgetId) {
        List<Transaction> transactionList = transactionService.getAllTransactionsByCustomerIdAndBudgetId(customerId, budgetId);
        return new ResponseEntity<>(transactionList, HttpStatus.OK);
    }

    @GetMapping("/{transId}")
    public ResponseEntity<Optional<Transaction>> getTransaction(@PathVariable long customerId, @PathVariable long budgetId, @PathVariable long transId) {
        Optional<Transaction> transaction = transactionService.getTransactionByCustomerIdAndBudgetId(customerId, budgetId, transId);
        return new ResponseEntity<>(transaction, HttpStatus.OK);
    }

    @PostMapping
    public ResponseEntity<Transaction> addTransaction(@PathVariable long customerId, @PathVariable long budgetId, @RequestBody Transaction transaction) {
        Transaction transactionToAdd = transactionService.addTransaction(customerId, budgetId, transaction);
        return new ResponseEntity<>(transactionToAdd, HttpStatus.CREATED);
    }

    @PutMapping("/{transId}")
    public ResponseEntity<Transaction> updateTransaction(@PathVariable long customerId, @PathVariable long budgetId, @PathVariable long transId, @RequestBody Transaction transactionDetails) {
        Optional<Transaction> updatedTransaction = transactionService.updateTransaction(customerId, budgetId, transId, transactionDetails);
        return updatedTransaction.map(transaction -> new ResponseEntity<>(transaction, HttpStatus.OK))
                .orElseGet(() -> new ResponseEntity<>(HttpStatus.NOT_FOUND));
    }

    @DeleteMapping("/{transId}")
    public ResponseEntity<Void> deleteTransaction(@PathVariable long customerId, @PathVariable long budgetId, @PathVariable long transId) {
        boolean isDeleted = transactionService.deleteTransaction(customerId, budgetId, transId);
        if (isDeleted) {
